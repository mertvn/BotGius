...
@error_handler_decorator()
async def ban_player(interaction: discord.Interaction, emq_name: str, is_banned: bool):
    """Interaction to handle the `/ban_player` command. It bans/unbans the `is_banned` field of the player with `name` == `emq_name`."""
    await interaction.response.defer(ephemeral=True)

    player_found, change_applied, player = Players_Controller().change_player_ban(emq_name, is_banned)
    if not player_found:
    ...
    if not change_applied:
        content = f'The change wasn\'t applied since the {player.emq_name}\'s ban value is already {is_banned}'
        await interaction.followup.send(content=content, ephemeral=True)
        return
    ...
    content = f'{interaction.user.mention} has {banned_value} {player.discord_ping} ({player.emq_name})'
    await log_thread.send(content=content, allowed_mentions=discord.AllowedMentions.none())
    await interaction.followup.send(content=f'{player.discord_ping} ({player.emq_name}) has been {banned_value} successfully!', ephemeral=True)
...
async def list_banned_players(interaction: discord.Interaction):
    ...
    banned_players_info = [f'{player.discord_ping} ({player.emq_name})' for player in banned_players]
